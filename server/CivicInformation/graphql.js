...
const CivicInformationGraphql = {
  ...
  resolvers: {
    Official: {
      tweets: async (parent, args, _context) => {
        ...
        let twitterId =
          parent.channels &&
          (parent.channels.find(channel => channel.type === 'Twitter') || {})
            .id;
        let tweets = [];
        if (twitterId) {
          tweets = await TwitterApi.getTweets(twitterId, limit).catch(
            err => []
          ); // return empty array on error.  I believe is due to incorrect twitter handles
        }
        tweets.splice(limit);
        return tweets;
      }
  ...
  queries: {
    officials: async (_parent, args, _context) => {
